...
UNEXPLORED=2;
ROBOT=5;
STARTING_POS_1=[17,12]; 
STARTING_POS_2=[32,12];
global ROWS; 
ROWS=23;
global COLS; 
COLS=49;
...
if step_num==0
    ...
    map=[ones(2,COLS)*WALL;  ones(ROWS-4,2)*WALL, ones(ROWS-4,COLS-4)*UNEXPLORED, ones(ROWS-4,2)*WALL;  ones(2,COLS)*WALL];
...
%     slams={ones(1,COLS)*17,ones(COLS-4,1)*5,ones(COLS-4,1)*21,ones(1,COLS)*5};

    mapPointBlocks1=zeros(ROWS,COLS);
    mapPointBlocks2=zeros(ROWS,COLS);

    chargingStations1=zeros(ROWS,COLS);
    chargingStations1(STARTING_POS_1(2),STARTING_POS_1(1))=1;

    chargingStations2=zeros(ROWS,COLS);
    chargingStations2(STARTING_POS_2(2),STARTING_POS_2(1))=1;
end
...
global COLS;
    ...
    GoalRegister=int8(zeros(ROWS,COLS));
